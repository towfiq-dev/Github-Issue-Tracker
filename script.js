...
function displayIssuesToUI(issuesArray) {
    ...
    issuesArray.forEach(function(singleIssue) {
        ...
        const cardContent = `
            <div onclick="showIssueDetails('${singleIssue._id}')">
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                    <span class="px-2 py-1 bg-gray-100 text-[10px] rounded text-gray-600 font-medium">${singleIssue.category}</span>
                    <span class="px-2 py-1 bg-yellow-50 text-[10px] rounded text-yellow-700 font-medium">${singleIssue.priority}</span>
                </div>

                <div class="mt-4 pt-3 border-t flex justify-between items-center text-[10px] text-gray-400">
                    <div>
                        <p class="font-semibold text-gray-600">${singleIssue.author}</p>
                        <p>${new Date(singleIssue.createdAt).toLocaleDateString()}</p>
                    </div>
                    <div class="text-right">
                        <span class="font-bold uppercase">${singleIssue.label}</span>
                    </div>
                </div>
            </div>
        `;
